...
class Solution(object):
    ...
    def search(self, matrix, target, r_left, r_right, c_left, c_right):
        ...
        if matrix[r_mid][c_mid] < target:
            ...
            return self.search(matrix, target, r_left, r_right, c_mid + 1, c_right) or self.search(matrix, target, r_mid + 1, r_right, c_left, c_mid)
        else:
            # the answer cannot be in the right botton corner of the current matrix
            return self.search(matrix, target, r_left, r_right, c_left, c_mid - 1) or self.search(matrix, target, r_left, r_mid - 1, c_mid, c_right)
